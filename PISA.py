...
class AttnProcessorDistReciprocal:
    ...
    def __call__(
        ...
        pisa_strength: Optional[float] = None,
        *args,
        **kwargs,
    ) -> torch.Tensor:
        # 显式声明 pisa_strength，diffusers 才会把 cross_attention_kwargs 里该键传入而非忽略
        if pisa_strength is None:
            pisa_strength = float(kwargs.pop("pisa_strength", 1.0))
        else:
            kwargs.pop("pisa_strength", None)
        pisa_strength = float(pisa_strength)
