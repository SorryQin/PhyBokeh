...
def get_cosine_schedule_with_warmup(
    ...
    def lr_lambda(current_step):
        ...
        return max(2e-7, 0.5 * (1.0 + math.cos(math.pi * float(num_cycles) * 2.0 * progress)))

    return LambdaLR(optimizer, lr_lambda, last_epoch)


def get_cosine_with_hard_restarts_schedule_with_warmup(
    optimizer: Optimizer, num_warmup_steps: int, num_training_steps: int, num_cycles: int = 1, last_epoch: int = -1, power: float = 1,
) -> LambdaLR:
    """
    Create a schedule with a learning rate that decreases following the values of the cosine function between the
    ...
    Args:
        ...
        power (`float`, *optional*, defaults to 1.0):
            The multiplier between cycles. Normally <= 1 for convergence.
    Return:
        `torch.optim.lr_scheduler.LambdaLR` with the appropriate schedule.
    """
    ...
    def lr_lambda(current_step):
        ...
        if progress >= 1.0:
            return 1e-6
        return max(1e-6, 0.5 * power**((float(num_cycles) * progress) // 1.0) * (1.0 + math.cos(math.pi * ((float(num_cycles) * progress) % 1.0))))
...
def get_scheduler(
    ...
    Args:
        ...
        power (`float`, *optional*, defaults to 1.0):
            Power factor. See `POLYNOMIAL` scheduler. If `COSINE_WITH_RESTARTS`, the power between cycles.
        last_epoch (`int`, *optional*, defaults to -1):
            The index of the last epoch when resuming training.
    """
