...
# 与 train_pisa_per_step_5t.py 对齐：同一前向内每个 denoise 子步单独计算 pisa_strength；默认 train_T_list 为 5 步 [499,400,300,200,100]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Inference：5 子步去噪 [499,400,300,200,100]，PISA 在子步间线性变化，对齐 train_pisa_per_step_5t.py。",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=[499, 400, 300, 200, 100],
        help="须与训练 train_pisa_per_step_5t.py 中 timesteps_list 一致；默认同名 5 步。",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="与 train_pisa_per_step_5t 一致：train_T_list 第一步的 PISA 比例。",
    )
    parser.add_argument(
        "--pisa_ratio_end",
        type=float,
        default=0.0,
        help="与 train_pisa_per_step_5t 一致：train_T_list 最后一步的 PISA 比例。",
    )
    parser.add_argument(
        "--step_update_scales",
        type=str,
...
def main():
    ...
    for batch in tqdm(val_dataloader):
        ...
        for blur_strength in blur_strength_list:
            ...
            # 推理（子步 pisa 与 train_pisa_per_step_5t 一致；默认每步保存解码图到 step_dir）
